...
export const MimeTypePicker = ({ value, onChange }: MimeTypePickerProps) => {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm">
      <div className="border-b border-neutral-200 bg-gradient-to-r from-neutral-50 to-white p-4 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div
            className={clsx(
              ...
              normalizedValue.length === 0 ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"
            )}
          >
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
          {MIME_PRESETS.map(preset => (
            <button
              ...
              className="group rounded-xl border border-neutral-200 bg-white p-3 text-left transition hover:border-indigo-300 hover:bg-indigo-50/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
            >
              <span className="flex items-center justify-between gap-2 text-xs font-semibold text-neutral-900">
                {preset.label}
                <span className="text-[10px] font-medium text-indigo-600 opacity-0 transition group-hover:opacity-100">
                  Use preset
                </span>
              </span>
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <label className="relative flex-1">
            ...
            <input
              ...
              className="h-10 w-full rounded-xl border border-neutral-300 bg-white pl-9 pr-3 text-sm outline-none transition placeholder:text-neutral-400 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"
            />
          </label>
          <button
            type="button"
                ...
                >
                  <span
                    className={clsx(
                      ...
                      isChecked ? "border-indigo-600 bg-indigo-600 text-white" : "border-neutral-300 bg-white"
                    )}
                  >
                    {isChecked ? (
                      <Check
        ...
        <div className="mt-5 rounded-xl border border-neutral-200 bg-neutral-50 p-3 sm:p-4">
          ...
          <div className="mt-3 flex gap-2">
            <input
              ...
              className="h-9 min-w-0 flex-1 rounded-lg border border-neutral-300 bg-white px-3 font-mono text-xs outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"
            />
            <button
              type="button"
              onClick={addCustomType}
              disabled={!customType.trim()}
              className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-neutral-900 px-3 text-xs font-medium text-white transition hover:bg-neutral-700 disabled:cursor-not-allowed disabled:opacity-40"
            >
        ...
        {normalizedValue.length > 0 ? (
          <div className="mt-4 flex justify-end">
            <button
              type="button"
              onClick={() => setTypes([])}
              className="text-[11px] font-medium text-amber-700 underline decoration-amber-300 underline-offset-4 hover:text-amber-900"
            >
